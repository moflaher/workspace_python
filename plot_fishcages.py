...
CS=ax_all.tricontour(data['trigrid'],data['h'],np.array([200]),color='black')
cb=plt.colorbar(trip,cax=cax)
cb.set_label(r'Depth (m)')
manual_locations = [(-70, 40),(-68.1,42.1),(-69.8,42.35),(-65,42)]
ax_all.clabel(CS, fontsize=12,fmt='%d', manual=manual_locations)
...
ax_bof.text(-67.5,45.35,'Passamaquoddy Bay',fontsize=14)
...
ax_cages.triplot(data['trigrid'],color='black',lw=.3,label='Mesh')
ax_cages.axis([-66.90,-66.815,45.025,45.06])
_formatter = mpl.ticker.ScalarFormatter(useOffset=False)
...
for i in cages:
    tnodes=data['nv'][i,:]    
    ax_cages.plot(data['nodell'][tnodes[[0,1]],0],data['nodell'][tnodes[[0,1]],1],'r',lw=.4,label='Cage')
    ax_cages.plot(data['nodell'][tnodes[[1,2]],0],data['nodell'][tnodes[[1,2]],1],'r',lw=.4,label='Cage')
    ax_cages.plot(data['nodell'][tnodes[[0,2]],0],data['nodell'][tnodes[[0,2]],1],'r',lw=.4,label='Cage')

handles, labels = ax_cages.get_legend_handles_labels()
handles=handles[::-1]
labels=labels[::-1]
legend=ax_cages.legend(handles[1:3], labels[0:2],loc=2)
t=legend.get_lines()
t[0].set_color('black')
