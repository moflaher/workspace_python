...
region=expand_region(region,[2500,2500],[0,0])
...
for i in range(9,10,1):
    ...
    for sub in range(1,2):#range(int(savelag1['x'].shape[0]/subset)):

        savepath='figures/timeseries/' + grid + '_' + datatype + '/lagtracker/subset/' + name + '_'+name2+'/'+regionname+'/' +lname +'/' +("%05d"%(subset*sub))+ '_'+("%05d"%(subset*(sub+1)))+ '/'
        if not os.path.exists(savepath): os.makedirs(savepath)

        pool = multiprocessing.Pool()
        pool.map(lag_plot,range(3700))
