...
def main():
    ...
    for i in range(1,20,1):
        ...
        for j in range(1,10,1):
            ax=f.add_axes([.125,.1,(i/100),(j/100)])
            plt.draw()
            axbb=ax.get_axes().get_position().bounds
            if ((np.fabs(axbb[2]-xtarget)<.01) and (np.fabs(axbb[3]-ytarget)<.01) ):
                break
                f.delaxes(ax)
            f.delaxes(ax)
    ...
    ax0f=[.125,.25,(i/100),(j/100)]
    ax1f=[ax0f[0]+ax0f[2]+.025,.25,(i/100),(j/100)]
    ...
    f.savefig(savepath + grid + '_' + regionname+'_current_variance_magnitude_diff_relative_subplot.png',dpi=150)
    #plt.close(f)
