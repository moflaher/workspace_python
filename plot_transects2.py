...
savepath='figures/png/' + grid + '_' + datatype + '/transects2/'
...
for key in trans.keys():
    ...
    cbax=ax.pcolormesh(dist,depth,speeda,vmin=clim[0],vmax=clim[1])
    ...
    f=plt.figure()
    ax=f.add_axes([.125,.1,.775,.8])
    cbax=ax.pcolormesh(dist,depth,speedb,vmin=clim[0],vmax=clim[1])
    cb=plt.colorbar(cbax)
    cb.set_label('Speed (ms$^{-1}$)')
    ax.set_xlabel('Distance (m)')
    ax.set_ylabel('Depth (m)')
    ...
    f=plt.figure()
    ax=f.add_axes([.125,.1,.775,.8])
    cbax=ax.pcolormesh(dist,depth,speeda-speedb,cmap=mpl.cm.seismic)
    cb=plt.colorbar(cbax)
    cb.set_label('Speed (ms$^{-1}$)')
    ax.set_xlabel('Distance (m)')
    ax.set_ylabel('Depth (m)')
