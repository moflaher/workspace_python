...
#        numberin2=np.sum(np.in1d(data['trigridxy_finder'].__call__(savelag2['x'][sidx,:],savelag2['y'][sidx,:]),cages).reshape(savelag2['x'][sidx,:].shape),axis=0)/npts
        numberin1=np.sum(p.contains_points(np.array([savelag1['x'][sidx,:].flatten(),savelag1['y'][sidx,:].flatten()]).T).reshape(savelag1['x'][sidx,:].shape),axis=0)/npts
        numberin2=np.sum(p.contains_points(np.array([savelag2['x'][sidx,:].flatten(),savelag2['y'][sidx,:].flatten()]).T).reshape(savelag2['x'][sidx,:].shape),axis=0)/npts
        ...
        sio.savemat('data/kelp_ratio_inregion/'+name+'_'+name2+'_'+regionname+'_'+lname+'_kelp_ratio_'+("%05d"%(subset*sub))+'_'+("%05d"%(subset*(sub+1)))+'.mat',mdict=savedic)
