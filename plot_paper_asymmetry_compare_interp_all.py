...
savepath='figures/png/' + grid + '_' + datatype + '/paper_asymmetry_subplots_interp/'+name+'_'+name2+'/'
...
ttidein=np.load('data/ttide/'+grid+'_'+name2+'_'+datatype+'_uv_all.npy')
ttidein=ttidein[()]
tidecon=ttidein['tidecon']
nameu=ttidein['nameu']
freq=ttidein['freq']
resmean=np.load('data/ttide/'+grid+'_'+name2+'_'+datatype+'_uv_all_mean.npy')
resmean=resmean[()]
m2i=np.where(nameu=='M2  ')
m4i=np.where(nameu=='M4  ')
SEMAZ0 = np.sqrt(resmean['resumean']**2+resmean['resvmean']**2);
...
#    ax1.add_collection(lseg1)
    ...
    f.savefig(savepath + grid + '_'+name+'_'+name2+'_' + regionname +'_paper_asymmetry.png',dpi=600)
    plt.close('all')
